...
/**
 * Class for representing the ACPIID
 *
 * @author Raveendra_Madala
 */
public class DCMACPIID {
     ...
     * constructed
     */
    public DCMACPIID(DCMACPIID inID) {
        if (inID != null) {
            mACPIID = inID.mACPIID;
        }
    }
    ...
    public int setID(String inID) {
        Pattern acpiIDPattern = Pattern.compile(mACPIIDPattern);
        Matcher matcher = acpiIDPattern.matcher(inID);
        if (!matcher.matches()) {
    ...
    private String mACPIID;
    private static final String mACPIIDPattern = "[A-Z0-9][A-Z0-9][A-Z0-9][A-Z0-9]";
}
