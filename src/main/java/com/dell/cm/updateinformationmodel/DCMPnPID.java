...
/**
 * Class for representing PnPID
 *
 * @author Raveendra_Madala
 */
public class DCMPnPID {
     ...
     * constructed
     */
    public DCMPnPID(DCMPnPID inID) {
        if (inID != null) {
            mPnPID = inID.mPnPID.toUpperCase();
        }
    }
    ...
    public int setID(String inID) {
        Pattern pnpIDPattern = Pattern.compile(mPnPIDPattern);
        Matcher matcher = pnpIDPattern.matcher(inID);
        if (!matcher.matches()) {
            return DCMErrorCodes.INVALID_PARAMETER;
        }
        mPnPID = inID.toUpperCase();
        return DCMErrorCodes.SUCCESS;
    }

    private String mPnPID;
    private static final String mPnPIDPattern = "[A-Z][A-Z][A-Z]";
}
